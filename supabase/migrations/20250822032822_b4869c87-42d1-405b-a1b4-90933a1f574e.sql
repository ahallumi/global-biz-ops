-- Phase 1: Database Foundation for Square POS Integration
...
CREATE TABLE product_pos_links (
  ...
  UNIQUE (source, pos_item_id, COALESCE(pos_variation_id,''), COALESCE(location_id,''))
);
...
CREATE INDEX idx_pos_links_source_item ON product_pos_links(source, pos_item_id);
